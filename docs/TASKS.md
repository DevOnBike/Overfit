...
| id | status | task |
|---|---|---|
| `RS-1` | **PART** | names confirmed live for both instruments used so far, and two traps found that a name check cannot see. `http.server.active_requests` is PUSHED (deltas, must be summed); `dotnet.monitor.lock_contentions` is **OBSERVABLE** — it reports nothing until `RecordObservableInstruments()` is called, and its value is absolute rather than a delta. Measured 2026-08-09: the subscription looked correct, the instrument reported as published, and the counter read a clean permanent **0** through sixteen contending threads. Callbacks must also be registered for `int` AND `long`, because one registered for the wrong width is silently never invoked. **Still open: the overhead half** — no measurement of what `MeterListener` costs on the lab-as-instrument |
...
| `RS-4` | **PART** | `dotnet.monitor.lock_contentions` deployed and bound as `LockContentions`; 12/12 pods report it, both configs agree (the `XC-7` gate enforces that now). **Separation is huge**: under a contended lock ON THE REQUEST PATH the rate went 0.076 -> 11.6/s, **152x**, while peers stayed at 0.05-0.08 and p95 went 48.8 -> 4911 ms. Floor from measurement: per-pod healthy peak 0.0514/s, x1.25 = 0.064, which sits ~190x below the fault. **But half the premise did not reproduce.** The plan justifies this channel as *"a latency cliff with every current channel quiet"*. GC was quiet (0.000007 -> 0.000008). **CPU was not**: 0.0025 -> 0.0204, an 8x rise and 2.6x above peers, so `CpuUsageRatio` would very likely have fired too. Likely cause is the reproduction rather than the claim — .NET monitors SPIN before blocking, so sixteen contending threads burn CPU that a real long-held lock would not — but that is a hypothesis and it is not measured. **The channel is justified as the SHARPER signal (152x against 2.6x), not as the only one.** Remaining: either reproduce contention that blocks without spinning, or amend the plan's premise |
