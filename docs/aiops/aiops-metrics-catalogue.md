...
customer"). All five ship in `k8s/anomaly-guard/guard.lab-workload.json`'s `customMetrics` block and the
matching `k8s/lab/anomaly-guard.yaml` ConfigMap; only `ActiveRequests` also ships in the `overfit`-namespace
config (`guard.lab.json`).
