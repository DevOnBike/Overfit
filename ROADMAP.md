...
| # | Item | Why it blocks |
|---|---|---|
| 2 | **Operator feedback** — marking an incident as noise feeds back into thresholds | Calibration is one-shot from a quiet window. Without a feedback path the only available response to a false alarm is muting the whole tool, which is exactly when products like this die. **Design: `docs/aiops-operator-feedback.md`.** |
