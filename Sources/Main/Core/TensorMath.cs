...
namespace DevOnBike.Overfit.Core
{
    public static class TensorMath
    {
        ...
        public static AutogradNode MSELoss(AutogradNode prediction, AutogradNode target)
        {
            ...
            using var diffBuffer = new FastBuffer<double>(n); // Zero-Alloc: ArrayPool pod spodem
            ...
            var finalLoss = TensorPrimitives.SumOfSquares((ReadOnlySpan<double>)diffSpan) / n;
        ...
        {
            int rows = logits.Data.Rows;
            int cols = logits.Data.Cols;
            ...
            for (int r = 0; r < rows; r++)
            {
                ...
                {
                    if (tRow[c] > 0.5) totalLoss -= Math.Log(pRow[c] + 1e-15);
                }
            }
        ...
        public static AutogradNode Conv2D(AutogradNode input, AutogradNode weights, int inC, int outC, int h, int w, int k)
        {
            ...
            {
                using var colData = new FastMatrix<double>(kSquareInC, outH * outW);
            ...
            if (outputNode.RequiresGrad)
                ComputationGraph.Active.Record(OpCode.Conv2D, outputNode, input, weights, new int[] { inC, outC, h, w, k });
                        ...
                        for (var ow = 0; ow < outputW; ow++)
                        {
                            ...
                            for (var ph = 0; ph < poolSize; ph++)
                            {
                                for (var pw = 0; pw < poolSize; pw++)
                                {
                                    ...
                                    if (val > maxVal) { maxVal = val; maxIdx = currentIdx; }
                                }
                            }
        ...
        public static AutogradNode GlobalAveragePool2D(AutogradNode input, int channels, int h, int w)
        {
            ...
            for (int b = 0; b < batchSize; b++)
            {
                for (var c = 0; c < channels; c++)
                {
                    ...
                    // Slice memory and use SIMD sum for speed
                    var spatialSpan = input.Data.ReadOnlyRow(b).Slice(offset, spatialSize);
                    sum = TensorPrimitives.Sum(spatialSpan);
            ...
            if (outputNode.RequiresGrad)
                ComputationGraph.Active.Record(OpCode.GlobalAveragePool2D, outputNode, input, null, new int[] { channels, h, w });

            return outputNode;
        }

        public static AutogradNode BatchNorm1D(AutogradNode input, AutogradNode gamma, AutogradNode beta, FastMatrix<double> runningMean, FastMatrix<double> runningVar, double momentum, double eps, bool isTraining)
        {
            ...
            var batchMeanMat = new FastMatrix<double>(1, C);
            var batchVarMat = new FastMatrix<double>(1, C);
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                for (var j = 0; j < C; j++)
                {
                    var xHat = (rowIn[j] - batchMeanRead[j]) * invStdVec[j];
                    rowXHat[j] = xHat;
            ...
            if (outputNode.RequiresGrad)
            {
                ComputationGraph.Active.Record(OpCode.BatchNorm1D, outputNode, input, null, new AutogradNode[] { gamma, beta, xHatTensor, invStdTensor });
            }
            else
            {
                xHatTensor.Dispose();
        ...
        public static void SoftmaxCrossEntropyBackward(AutogradNode logits, AutogradNode target, AutogradNode output)
        {
            ...
            int rows = logits.Data.Rows;
            int cols = logits.Data.Cols;
            double scale = output.Grad.AsReadOnlySpan()[0] / rows;
        ...
        public static void Im2Col(ReadOnlySpan<double> input, int channels, int height, int width, int kSize, int stride, int padding, Span<double> output)
        {
        ...
        public static void Col2Im(ReadOnlySpan<double> colData, int channels, int height, int width, int kSize, int stride, int padding, Span<double> gradInput)
        {
            gradInput.Clear();
            var outH = (height + 2 * padding - kSize) / stride + 1;
            var outW = (width + 2 * padding - kSize) / stride + 1;
