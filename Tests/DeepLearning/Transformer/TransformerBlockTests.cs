...
namespace DevOnBike.Overfit.Tests.DeepLearning.Transformer
{
    public class TransformerBlockTests
    {
        ...
        public void GELU_Backward_NumericalGradientCheck()
        {
            ...
            foreach (var x in xs)
            {
                ...
                Assert.True(relErr < 0.01f, $"GELU grad at x={x}: analytical={analytical:F6}, numerical={numerical:F6}");
            }
        }
        ...
        public void TransformerBlock_ParameterCount_IsCorrect()
        {
            ...
            var expected =
                2 * dModel + // LN1
                4 * dModel * dModel + 4 * dModel + // MHA: Wq/Bq, Wk/Bk, Wv/Bv, Wo/Bo
                2 * dModel + // LN2
        ...
        public void TransformerBlock_SaveLoad_ProducesSameOutput()
        {
            ...
            {
                Assert.True(MathF.Abs(s1[i] - s2[i]) < Tolerance, $"[{i}] after load: {s1[i]:F6} vs {s2[i]:F6}");
            }
        }
