...
namespace DevOnBike.Overfit.Core
{
    public class FastTensor<T> : IDisposable
    {
        ...
        public T this[int i, int j, int k] { get => _data[Offset + i * Strides[0] + j * Strides[1] + k * Strides[2]]; set => _data[Offset + i * Strides[0] + j * Strides[1] + k * Strides[2]] = value; }
        public T this[int i, int j, int k, int l] { get => _data[Offset + i * Strides[0] + j * Strides[1] + k * Strides[2] + l * Strides[3]]; set => _data[Offset + i * Strides[0] + j * Strides[1] + k * Strides[2] + l * Strides[3]] = value; }

        public Span<T> AsSpan()
        {
            if (!IsContiguous) throw new InvalidOperationException("Użyj ToContiguous() najpierw, tensor nie jest ciągły w pamięci.");
        ...
        public FastTensor<T> Transpose(int dim0, int dim1)
        {
            ...
            // Zamieniamy miejscami wymiary i kroki pamięci
        ...
        public FastTensor<T> ToContiguous()
        {
            ...
            for (var i = 0; i < Size; i++)
            {
                ...
                for (var d = 0; d < Rank; d++) sourceOffset += indices[d] * Strides[d];
        ...
        {
            if (CalculateSize(newShape) != Size) { throw new ArgumentException("Shape mismatch."); }
