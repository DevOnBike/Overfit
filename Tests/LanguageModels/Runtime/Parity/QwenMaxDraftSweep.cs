    ...
    public sealed class QwenMaxDraftSweep
    {
        ...
        public void Sweep_MaxDraft_OnEchoText()
        {
            ...
            (double tps, double acc) Spec(int md)
            {
                using var s = engine.CreateSession(512);
                s.Reset(promptArr);
                var h = new List<int>(promptArr);
                var buf = new int[md + 2];
                int p = 0, steps = 0;
                var sw = System.Diagnostics.Stopwatch.StartNew();
                while (p < generate)
                {
                    var n = s.GenerateSpeculative(CollectionsMarshal.AsSpan(h), buf, maxDraft: md);
                    for (var c = 0; c < n; c++)
                    {
                        h.Add(buf[c]);
                    }
                ...
                sw.Stop();
                return (p / sw.Elapsed.TotalSeconds, (double)p / steps);
            }

            var single = Single();
            _out.WriteLine($"single (baseline) = {single:F2} tok/s");
            foreach (var md in new[] { 1, 2, 3, 4, 6, 8, 12 })
            {
                var (tps, acc) = Spec(md);
                _out.WriteLine($"maxDraft={md,2}: {tps,6:F2} tok/s  speedup={tps / single:F2}x  accept={acc:F2}/step");
            }
        }
    }
}
