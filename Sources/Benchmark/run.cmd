...
@rem The exit code is the only thing a caller can read, and three of its values mean different things.
...
@if not "%BENCH_RC%"=="2" if not "%BENCH_RC%"=="3" echo FAILED (exit %BENCH_RC%): the benchmark run itself failed.
@exit /b %BENCH_RC%
