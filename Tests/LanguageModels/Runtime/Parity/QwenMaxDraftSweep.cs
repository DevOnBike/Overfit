...
namespace DevOnBike.Overfit.Tests.LanguageModels.Runtime.Parity
{
    public sealed class QwenMaxDraftSweep
    {
        ...
        public void Sweep_MaxDraft_OnEchoText()
        {
            ...
            var prompt = new List<int>(); for (var r = 0; r < 10; r++) prompt.AddRange([785, 4062, 13283, 38835, 35308, 916, 279, 16704, 5562, 13]);
            var promptArr = prompt.ToArray();
            const int generate = 120;

            double Single() { using var s = engine.CreateSession(512); s.Reset(promptArr); var sa = SamplingOptions.Greedy;
            ...
            (double tps, double acc) Spec(int md) { using var s = engine.CreateSession(512); s.Reset(promptArr); var h=new List<int>(promptArr); var buf=new int[md+2]; int p=0,steps=0;
              var sw=System.Diagnostics.Stopwatch.StartNew(); while(p<generate){ var n=s.GenerateSpeculative(CollectionsMarshal.AsSpan(h), buf, maxDraft:md); for(var c=0;c<n;c++) h.Add(buf[c]); p+=n; steps++; } sw.Stop(); return (p/sw.Elapsed.TotalSeconds, (double)p/steps); }
