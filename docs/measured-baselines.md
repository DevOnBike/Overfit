...
## `XC-131` / `XC-123`: embeddings pooled the wrong tensor, and the repair is measurable against two independent references — 2026-08-27

**What was wrong.** `CachedLlamaSession.Embed` pooled the hidden state BEFORE the final norm. Every
reference implementation — HuggingFace's `last_hidden_state`, llama.cpp's `llama-embedding` — means the
state AFTER it. The final norm's per-channel gain is strongly anisotropic (Qwen3-Embedding-0.6B:
**-0.1196 to 15.3125**), so the two are different directions, not a rounding apart.

**Measured on:** `C:\qwen3-embed\Qwen3-Embedding-0.6B-Q8_0.gguf`, 639,150,592 bytes, SHA-256 of the first
mebibyte `9220223cd8a60ae48d03e25ee8b495fa82e1d534934a6b5a1d76ebd0f54a4964`. Reference producer:
`llama-embedding.exe`, `version: 0.3.0-dev (build 1411, commit d7a20741)`, MSVC 19.44.35228.0, x64, built
from `D:\llamacpp-tmp\build-avx2` with `-t 16`, `--embd-normalize 2`. Overfit side: .NET 10, Release,
Ryzen 9 9950X3D, Windows. Both engines read the **same quantised bytes**, so a disagreement is arithmetic
and not quantisation. The reference vectors are committed at
`Tests/test_fixtures/qwen3_embedding_llamacpp_reference.json` and regenerated by
`Scripts/qwen3_embedding_reference.py`.

### Cosine against llama.cpp, last-token pooling, four texts

| pooled tensor | text 0 (en) | text 1 (pl, ASCII) | text 2 (pl, `ż`) | text 3 (instruction-prefixed) |
|---|---|---|---|---|
| **post-norm (`quantize:false`, shipped)** | 0.999560 | 0.999479 | 0.999487 | 0.999644 |
| post-norm, `quantize:true` | 0.999417 | 0.999460 | 0.999434 | 0.999672 |
| pre-norm (the defect) | 0.941061 | 0.976880 | 0.990304 | 0.987849 |
| post-norm, EOS not appended | 0.796002 | — | — | — |

### Mean pooling — the shipped default of `Embed`, and what `OverfitClient.Embed`, `McpRagIndex` and the ASP.NET demo consume

Against llama.cpp `--pooling mean` on the identical token streams:

| arm | text 0 (8 tok) | text 1 (11 tok) | text 2 (11 tok) | text 3 (26 tok) |
|---|---|---|---|---|
| **before the fix** (pre-norm) | 0.891189 | 0.844347 | 0.854580 | 0.827365 |
| after, `quantize:true` | **0.999889** | **0.999755** | **0.999755** | **0.999576** |
| after, `quantize:false` | 0.999211 | **0.997530** | **0.997622** | 0.999525 |

**The quantisation arms separate on MEAN pooling and not on last-token pooling, by up to 10x in
deviation-from-1** (text 1: 2.470e-3 against 2.45e-4). That is the position-0 divergence below reaching
the vector `XC-131` ships, diluted by 1/n but not absent.

### The HuggingFace tiebreak, which shares nothing with llama.cpp

The model card publishes the similarity matrix its own full-precision PyTorch model produces for four
named texts. Reproduced end to end through the query instruction prefix, last-token pooling, the appended
end-of-text token and L2 normalisation — **worst absolute deviation over the four pairs:**

| arm | worst \|delta\| |
|---|---|
| `quantize:false` | **0.000988** |
| `quantize:true` | 0.002733 |
| end-of-text token NOT appended | **0.106111** |

That last row is the whole reason the EOS question is settled rather than argued: llama.cpp and Overfit
both append it, so they agree with each other either way, and only an oracle outside both can say which is
right.

### Per-vector parity does NOT imply pairwise parity — 18x apart

`cos(text0, text2)` computed on our vectors against the same quantity computed on llama.cpp's:

| arm | our pairwise | llama.cpp | delta |
|---|---|---|---|
| `quantize:false` | 0.724227 | 0.723768 | **4.6e-4** |
| `quantize:true` | 0.715279 | 0.723768 | **8.5e-3** |
...
Peak working set of a process that loads the 0.6B file and embeds one query, 1024-token context, two runs
per arm (they agree to 1 MB):

| arm | after load | peak working set |
|---|---|---|
| `quantize:false` | 3040.5 MB | **3270.4 / 3270.7 MB** |
| `quantize:true` | 1378.8 MB | **1607.4 / 1608.3 MB** |

2.03x, and the KV cache is ~229 MB of each. **Affordable for 0.6B and not a general licence:** the same
...
| text | `quantize:false` | `quantize:true` |
|---|---|---|
| 0 | 0.9786 | 0.999997 |
| 1 | 0.9064 | 0.999979 |
| 2 | 0.9064 | 0.999979 |
...
Per-layer capture at position 0 puts all of it in the LAST transformer block: the absolute difference
between the two arms' residual streams is **flat at 31.2 from layer 2 to layer 26 and jumps to 765.5 at
layer 27**. Block 27 is where the model cancels its massive-activation channel — channel 35 carries 5704.5
at layer 26 and the block subtracts 6701 (`quantize:false`) or 6061 (`quantize:true`). So a **0.52% input
difference emerges as a 10.2% difference in the block's output magnitude**, its direction still agreeing at
cosine 0.99762.

**It reaches the shipped mean-pooled vector, and the attribution is measured rather than argued.** Remove
position 0 from BOTH arms' means and they collapse together:

| arm | text 0 | text 1 | text 2 | text 3 |
|---|---|---|---|---|
| mean **excluding position 0**, `quantize:false` | 0.999846 | 0.999682 | 0.999728 | 0.999595 |
| mean **excluding position 0**, `quantize:true` | 0.999875 | 0.999727 | 0.999726 | 0.999547 |

Largest remaining gap **4.5e-5**, and on texts 2 and 3 the dequantised arm is the better one. Two
reconstruction checks were run first, because without them the ablation could be measuring the harness:
our mean-of-per-position rebuilt `Embed(Mean)` at cosine **1.000000**, and llama.cpp's mean-of-per-token
rebuilt its own `--pooling mean` output at **1.000000**. Per-position states were obtained by embedding
each prefix of the token stream with last-token pooling, which is exact for a causal model.

**Length is NOT what decides.** Text 0 is the shortest at 8 tokens and has the second-smallest gap: the
