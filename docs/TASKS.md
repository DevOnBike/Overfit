...
| `AN-E2` | **DONE** | throttling is detectable, and `blind` is **0 for the first time in this project**. Four checks, written down before measuring: (1) `CpuThrottleRatio` bound, read back out of the deployed ConfigMap; (2) **blind 1 -> 0** — it had been 1 in every cycle, and binding was the missing half: the CFS series existed once the workload carried a CPU limit, but the config never listed the channel so the guard issued no query; (3) healthy population quiet — peers at **0.0% for the whole eight-minute fault**, and at 1000m a healthy pod sits at 14.6-19.0% of the window above the 5% threshold against the rule's 25%; (4) **fires on the fault** — target throttling 71.6% -> 100%, incident opened *"Held at or above 0.05 for 52% of the window, 31 of 60 observations"*, with the peer family raising the same pod independently and no peer accused. **The CPU limit is the enabling change**: CFS accounting only exists on a container with a quota, so without one the channel could never report. 1000m because `SustainedThresholdOptions.ForCpuThrottling` was calibrated on a pod limited to one core — a threshold describes the regime it was measured in. **Two of my own errors cost iterations**: the limit was first sized at 200m from AVERAGE cpu when CFS throttles on bursts inside a 100 ms period, and the fault was first sized for the 200m limit and not recomputed when the limit moved to 1000m — 0.32 cores of demand against a 1-core quota cannot throttle. Unblocks `PS-3` |
...
| `SEC-1` | **OPEN** | **the guard's `POST /ack` is unauthenticated and shares port 9469 with the metrics scrape.** Found by `overfit-ciso` 2026-08-09 in `Sources/Cli/GuardMetricsEndpoint.cs`. The manifest ships no `NetworkPolicy`, so on a cluster without default-deny anyone with pod-level network reach can **silence a real incident for up to 90 days with one HTTP call**. Judged disclosable rather than embargoed — it is visible from the endpoint's own 404 body and the fix needs no code change. Two more manifest gaps by omission from the same review: no `automountServiceAccountToken: false` and no `securityContext` on the guard Deployment. All three are YAML |
