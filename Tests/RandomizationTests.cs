...
namespace DevOnBike.Overfit.Tests
{
    public class RandomizationTests
    {
        ...
        public void PRNG_StatisticalTest_UniformDistributionQuality()
        {
            ...
            double mean = buffer.Average(x => (double)x);
            Assert.InRange(mean, 0.499, 0.501); // Bardzo blisko 0.5

            // Assert 3: Wariancja (~0.08333)
            double variance = buffer.Average(x => Math.Pow(x - mean, 2));
            ...
            foreach (var value in buffer)
            {
                ...
                if (bucketIndex >= numBuckets) bucketIndex = numBuckets - 1; // safety
                buckets[bucketIndex]++;
            }

            // Spodziewamy się 100,000 elementów w każdym z 10 wiader
            int expectedPerBucket = sampleSize / numBuckets;
            int tolerance = (int)(expectedPerBucket * 0.015); // Tolerancja odchylenia 1.5%
