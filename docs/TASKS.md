...
| `AN-D8` | **OPEN** | **`OomEventsRate` is bound to a series that is permanently zero, and nothing reports it as a problem.** Measured 2026-08-08 against a confirmed OOMKill: `container_oom_events_total{pod=<the killed pod>}` = **0**, and across **43 series cluster-wide over an hour the only distinct value is 0.0** — cAdvisor does not populate it on this runtime. The fact IS available: `kube_pod_container_status_last_terminated_reason{reason="OOMKilled"}` = 1 for exactly that pod, from kube-state-metrics, which the guard does not read. **Why this is worse than `AN-D7` and not merely equal**: a channel with no series is counted as `blind` and is visible in every cycle line; a channel pinned at 0 reports a number, looks healthy, satisfies coverage checks, and can never fire. The detection of a real OOM came entirely from `ContainerRestarts`. Fix: rebind to the kube-state-metrics series, and add a check for a channel whose value never varies across the whole history — the same shape as the latency-bucket geometry in `AN-F2` |
