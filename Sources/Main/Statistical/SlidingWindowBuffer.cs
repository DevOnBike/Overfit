...
namespace DevOnBike.Overfit.Statistical
{
    ...
    public sealed class SlidingWindowBuffer : IDisposable
    {
        ...
        /// Zero allocation — WriteFeatureVector writes directly to the ring slot.
        /// </summary>
        ...
        public void Add(ReadOnlySpan<float> features, DateTime timestamp)
        {
            ...
            {
                throw new ArgumentException($"Expected {_featureCount} features, got {features.Length}.", nameof(features));
            }
        ...
        public bool TryGetWindow(Span<float> destination, out DateTime windowEnd)
        {
            ...
            {
                throw new ArgumentException($"Destination too short: needs {_windowFloats}, available {destination.Length}.", nameof(destination));
            }
        ...
        public bool TryGetWindow(FastMatrix<float> destination, out DateTime windowEnd)
        {
            ...
            {
                throw new ArgumentException($"FastMatrix must be [{_windowSize}×{_featureCount}], got [{destination.Rows}×{destination.Cols}].", nameof(destination));
            }
        ...
        private bool TryGetWindowCore(Span<float> destination, out DateTime windowEnd)
        {
            lock (_lock)
            {
                ...
                // Safe index resolution without modulo
        ...
        {
            // OPTIMIZATION: Removed slow modulo (%) operator. 
            // Simple branch prediction handles this perfectly.
            _head++;
