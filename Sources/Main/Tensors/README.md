...
lifetimes and `RentArray`/`ReturnArray` for class-lifetime buffers. The wrapper exists so rentals are
visible and paired; raw pool calls have leaked here before. `OverfitPool<T>`, an earlier attempt at a
...
**Jagged `float[][]` is banned in this project** (`OVERFIT-JAGGED`, build error). Use a flat `float[]`
