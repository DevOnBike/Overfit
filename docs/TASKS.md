...
| `XC-7` | **OPEN** | **`kubectl apply` silently REMOVED a live binding, and reported "configured".** Measured 2026-08-08/09: `k8s/lab/anomaly-guard.yaml` never carried a `customMetrics` block while `k8s/anomaly-guard/guard.lab-workload.json` did, so applying the YAML deleted the deployed `GcCommittedBytes` channel — `RS-2`, recorded DONE. Nothing reported it: the channel simply stopped being evaluated, and a channel that is not evaluated produces no findings, which is what a healthy cluster also produces. Found by accident while binding `ActiveRequests`. **This is a worse shape than the known `kubectl` trap**: that one is "the action did not happen"; this one is "the action happened and quietly took something away". `k8s/README.md` already says the two files must agree — the rule existed and nothing enforced it. Fix: a check that the ConfigMap's embedded `guard.json` and the standalone config declare the same channel set, run in the suite so it cannot drift again. Both files now carry `GcCommittedBytes` + `ActiveRequests`, verified by reading the ConfigMap back out of the cluster |
