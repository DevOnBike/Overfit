...
{
    /// <summary>
    /// Experimental data-parallel TinyShakespeare trainer.
    ///
    /// Purpose:
    /// use more CPU cores during training without making ComputationGraph,
    /// Parameter.Grad, or optimizer state thread-shared.
    ///
    /// Design:
    ///
    /// master model:
    ...
    ///
    /// worker models:
    ///   - each owns its own GPT1Model,
    ///   - each owns its own ComputationGraph,
    ///   - each samples and trains on a local mini-batch,
    ///   - gradients are reduced into master after Parallel.For completes.
    ///
    /// This avoids racing on:
    ///   - ComputationGraph tape,
    ///   - AutogradNode gradients,
    ///   - Parameter.GradSpan,
    ///   - Adam state.
    ///
    /// Paths intentionally match the existing TinyShakespeare tests:
    ///
    ...
    ///
    /// PowerShell:
    /// $env:OVERFIT_TINY_SHAKESPEARE_DP_STEPS="1250"
    /// $env:OVERFIT_TINY_SHAKESPEARE_DP_WORKERS="8"
    /// dotnet test -c Release --filter "Demo_Train_TinyShakespeare_DataParallel_AndWriteCheckpointBin"
    ///
    /// 1250 steps * 8 workers * local batch 8 == 80k sequences,
    /// same number of sequences as the previous 10k single-worker batch-8 run.
    /// </summary>
    public class TinyShakespeareDataParallelTrainingTests
    {
        ...
        private const int DefaultDataParallelSteps = 5_000;
        ...
        public void Demo_Train_TinyShakespeare_DataParallel_AndWriteCheckpointBin()
        {
            ...
            try
            {
                ...
                CopyParametersToWorkers(
                    masterParameters,
                    workers);

                _output.WriteLine("=== Overfit TinyShakespeare Data-Parallel Training Demo ===");
                ...
                for (var step = 0; step < steps; step++)
                {
                    ...
                    ClearMasterGradients(masterParameters);
            ...
            int[] corpus)
        {
            ...
            ClearWorkerGradients(worker.Parameters);
        ...
        private static void ClearMasterGradients(
        ...
        private static void ClearWorkerGradients(
            IReadOnlyList<Parameter> parameters)
        {
            for (var i = 0; i < parameters.Count; i++)
            {
