...
| `AN-F1` | **PART** | **two explanations eliminated, one real defect found — and it is not the cause.** `SeasonalExpectationNoiseDiagnostics` reproduces the correction deterministically on a smooth synthetic day. (1) **Anchor noise is not the mechanism**: 134/133/139/139/138/135 findings across 2 to 7 days of history — flat, so more days fix nothing. (2) **The interpolation SHAPE is a real defect**: on a perfectly smooth curve the correction injects a finding in **222 of 284 windows**, because a straight chord subtracted from a curve leaves a residual that is perfectly monotone inside a twenty-minute window — exactly the pattern a rank test is built to be confident about. The raw column is 0 throughout; every one of those is manufactured by the correction. (3) **But it cannot explain the lab.** The effect decays with signal scatter — 222, 178, 133, 52, 2, 0 as noise goes 0 → 2.5% of daily amplitude — and vanishes above about **1%**. Measured on the lab's `RequestsPerSecond` over three hours: within-window scatter is **7.8% of amplitude** (7.0–10.2%), eight times too much. **So the 11 → 33 remains unexplained** and the leading hypothesis is now dead. Next candidate, untested: not a slope error but a LEVEL one — anchors median across days of different absolute traffic, so subtraction shifts the level and the relative gate is measured against a much smaller "typical" |
