...
| `AN-D2` | **DONE** | answered 2026-08-08 with a real OOMKill (`OOMKilled`, exit 137) on a 12-replica lab. **The row's claim was half right.** The peer family IS blind to a single OOMKill — confirmed. But the guard is not: the absolute rule on `ContainerRestarts` opened an incident two cycles later (*"Held above zero for 20% of the window, 7 of 35 observations"*, severity 0.2) with three related findings — `GcGen2HeapBytes` 60% **below** peers and `GcCommittedBytes` 45% below (a cold heap after restart) and `CpuUsageRatio` 49% above (warm-up). **`OomEventsRate` contributed nothing**, which is `AN-D8` |
...
| `AN-D8` | **PART** | **deployed 2026-08-08 23:5x.** `OomEventsRate` now reads kube-state-metrics: `increase(restarts_total[w]) * on (namespace,pod,container) group_left() last_terminated_reason{reason="OOMKilled"}` — chosen by measurement against the real kill, not by reasoning: it returned 1.00 for the killed pod and no series for the other eleven, and it **decays** (0 at 2/5/15 min, 1.00 at 30 min), which the reason flag alone does not — that is a state persisting for the container's life and would hold a pod anomalous for ever. **Fixing the template was not enough and that was the trap**: a configured binding overrides it, so every real deployment would have kept the dead series, and deleting the entry is worse (an unbound channel is blind). Added a verbatim `query` field — a join cannot be expressed as name-plus-kind — rejected by the reader unless it carries `%selector%`, or it would silently report on the whole cluster. Also `InertChannels()`: channels that never once moved, split by whether the constant is non-zero (**conclusive** — no real measurement repeats bit-exactly for hours, e.g. latency 48.75) or zero (**ambiguous** — what a healthy rare-event channel looks like). Four mutations, each caught by the right test; suite 2488/2219/269/0. Deployed and verified: ConfigMap read back OUT of the cluster before restarting onto it (apply exits 0 on a silently dropped field), pod digest matches the locally built image exactly, first cycle `pods=12 blind=1 unevaluable=1` and **no `Blind on OomEventsRate` line**. **What that proves and what it does not**: the channel binds and is queried. It has NOT been shown to FIRE — the only event available decayed out of the window an hour before deployment, and the guard restarted with no learned history, so injecting now would confound a fresh-guard effect with the binding. **Remaining: one `POST /fault/oom` once the guard has history**, which closes D2, D6 and D8 end to end |
