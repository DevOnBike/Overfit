...
**Recommendation (coordinator, 2026-08-08 — a recommended default, not a client decision): both, in sequence, not either/or.** `CustomMetricBinding` first — its own doc scopes it to "a metric this project does not model, that a deployment wants watched anyway," and PSI does not fit that description; it is a universal container signal whose narrower, permanently-blind sibling `CpuThrottleRatio` is already first-class. But config-only is right *first* regardless, because it works on the already-deployed build with no release, and it lets the starvation floor (Scope, point 3) be measured before a channel ships carrying a guessed one. Promote to a `MetricIndex` member afterwards — cheap, per the `ContainerRestarts` precedent above — once the floor exists and the client has said this is a permanent channel, not just this lab's config. This sequencing is carried into Open question 1 below as the default if the client does not answer.
...
4. **Does E2 become unnecessary?** No — recommend keeping it, and the induced-contention experiment this plan needs (point 3) *justifies* E2 rather than replacing it: throttle still needs pods with a CPU limit, which PSI specifically does not, so the two remain complementary test-infrastructure needs, not overlapping ones. Suggest E2's scope note be updated (by the developer/architect, not here) to say the same experiment now serves both channels.

### 4a. `guard.lab.json` vs `guard.lab-workload.json` — the relationship, established rather than assumed

Neither file is referenced by any script, manifest or test in this repo (`grep` across the tree found zero hits outside this plan) — both are read only by a human passing `overfit anomaly-guard --config <path>` by hand. Reading both plus the lab's own docs establishes:

- `guard.lab.json` targets `namespace: overfit`, `podRegex: overfit-server-.*` — this project's own ASP.NET server. `Demo/LabWorkload/README.md` states this deployment was **replaced**: *"The previous lab ran the real Overfit server, which was a poor instrument: a 1.07 GB model per pod capped the node at about four replicas, only one fault was injectable... Twelve replicas of a small app answer questions four replicas of a large one cannot."* `Tests/Anomalies/Diagnostics/LabFixtureRecorderDiagnostics.cs:61-65` independently confirms this in a bug report: an early recorder hardcoded `overfit-server-.*` and "stopped matching the moment the lab was rebuilt" around `labapp_*`.
- `guard.lab-workload.json` targets `namespace: lab`, `podRegex: lab-workload-.*`, `labapp_*` — the current `LabWorkload` synthetic fault-injection app. The scaling experiment cited in the backlog ("12 -> 15 -> 12") matches a many-small-replica topology, not the four-replica cap the README states for the old deployment — consistent with the PSI measurement having been taken against `lab-workload-*`, not `overfit-server-*`.
- **Neither committed file is what diagnostics actually read by default.** `LabFixtureRecorderDiagnostics.cs:67-69` and three other diagnostics resolve their config from `OVERFIT_LAB_CONFIG`, defaulting to `D:/Overfit/Tests/bin/lab-guard.json` — an **untracked, operator-maintained build-output file**, not under `k8s/anomaly-guard/`. Read directly: it targets `lab-workload-.*`/`labapp_*` (matching `guard.lab-workload.json`'s shape) and additionally carries **operator-calibrated** `minGap`/`minTrendChange` values (e.g. `"CpuUsageRatio": {"minGap": "0.00041", "minTrendChange": "0.000326"}`) that neither committed file has.

**Conclusion**: `guard.lab.json` is a stale worked example for a deployment the lab README says was superseded — flagged for the developer/architect, not fixed here. `guard.lab-workload.json` is the right committed file to add the PSI binding to; it is the closest tracked ancestor of what the lab actually runs. **Activating the binding on the live lab additionally requires the operator to mirror the same entry into their local, untracked `Tests/bin/lab-guard.json`** — that step is outside this repository's version control and outside what any plan here can "ship"; it is a runbook line for whoever operates the lab, not a task for the developer.
...
- No change to `guard.lab.json` (established stale, see 4a) or the production ServiceMonitor beyond what's needed to scrape the new series.
...
| Fact | `guard.lab.json` (`overfit-server-.*`) targets a deployment the lab's own README (`Demo/LabWorkload/README.md`) and a bug report in `LabFixtureRecorderDiagnostics.cs:61-65` both describe as superseded by `LabWorkload` (`lab-workload-.*`/`labapp_*`, matching `guard.lab-workload.json`). Verified by reading both files plus the untracked live config below — not assumed. |
| Fact | The config diagnostics actually read by default is `Tests/bin/lab-guard.json` (via `OVERFIT_LAB_CONFIG`, `LabFixtureRecorderDiagnostics.cs:67-69`), untracked, and matches `guard.lab-workload.json`'s shape plus operator-calibrated thresholds — read directly, not inferred. |
...
| Assumption | `waiting` alone (not `stalled`) is the right first slice. **Coordinator-recommended default, 2026-08-08, not client-confirmed** — grounded in the measured near-identical p50s and the grouper counting distinct signals (two channels for one phenomenon inflates finding count). |
| Assumption | No built-in `SustainedThresholdRule`/absolute floor ships in this slice — observable-only until an induced-contention measurement exists. **Coordinator-recommended default, 2026-08-08, not client-confirmed.** |
| Assumption | Vehicle sequencing — `CustomMetricBinding` now, `MetricIndex` member after the floor is measured. **Coordinator-recommended default, 2026-08-08, not client-confirmed**; supersedes the analyst's original single-vehicle recommendation. See "The central design question" above. |
| Decision | client, 2026-08-08 — add PSI **alongside** `CpuThrottleRatio`, not replacing it. |
| Constraint | A new `MetricIndex` member, if/when that route is taken, must be appended after `ContainerRestarts` (ordinal ≥13) — inserting earlier silently reinterprets every persisted sample (`MetricIndex.cs:26-28`, the file's own warning). |
...
| Fact | `guard.lab.json` targets a superseded deployment (`overfit-server-.*`); `guard.lab-workload.json` targets the current one and is the right file for this binding; the live lab's actual config is a third, untracked file. Established in 4a above — no longer an open question. |
| Open question | **For the client** — confirm or override the vehicle-sequencing default (point 1 above). Owed before any code is written. |
| Open question | **For the client** — confirm or override the `waiting`-only default (point 2 above). |
| Open question | **For the client/architect** — confirm or override the observable-only/deferred-floor default (point 3 above). |
...
**Recommended defaults below are the coordinator's, dated 2026-08-08 — the client has not confirmed any of them. They are carried as the assumption to build against, not recorded as decisions, and every question stays open until the client actually answers it.**

1. **Vehicle**: ship this as a `CustomMetricBinding` config entry, as a new `MetricIndex` member, or both in sequence? **Recommended default (coordinator): both — `CustomMetricBinding` now, promoted to a `MetricIndex` member once the floor is measured**, not an either/or. Reasoning: `CustomMetricBinding`'s own doc scopes it to "a metric this project does not model, that a deployment wants watched anyway" and PSI does not fit that description (it is universal, not customer-specific) — but config-only is right *first* regardless, because it ships on the already-deployed build with no release and lets the floor be measured before a channel carries a guessed one; promoting later is cheap, shipping a guessed floor is not. If the client answers with a single vehicle instead of the sequence, that supersedes this default.

2. **Channel count**: `waiting` only, or `waiting` + `stalled`? **Recommended default (coordinator): `waiting` only.** Measured: the two track each other on this lab (p50 1.39e-5 vs 1.38e-5), and the grouper counts *distinct* signals — two channels moving together for one phenomenon inflates the finding count rather than describing it more precisely.

3. **Calibration scope**: is an induced-CPU-contention experiment in scope now, or does the channel ship observable-only with the floor deferred? **Recommended default (coordinator, agreeing with the analyst's original reasoning): observable-only; defer the floor.** Addendum: the experiment this eventually needs *justifies* backlog E2 rather than removing it — throttle still needs CPU-limited pods, which PSI specifically does not, so the two stay complementary.

4. **Scope config**: **established, not open any more (see 4a above).** `guard.lab.json` targets a superseded deployment; `guard.lab-workload.json` is the right committed file for this binding; the live lab additionally needs the untracked `Tests/bin/lab-guard.json` updated by whoever operates it, which is outside this repository's version control.

5. **`SignalClass`** (for the developer/architect to confirm, not the client): is `Infrastructure` the right class for PSI, by the same reasoning `SignalClass.Infrastructure`'s own doc comment gives for CFS throttling? Coordinator confirms this routing (architect's call, not the client's). If unanswered, the plan assumes **`Infrastructure`**, on the stated precedent.
