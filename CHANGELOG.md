...
- **In-repo Roslyn performance analyzer suite (`Sources/Analyzers`, 10 rules `OVERFIT001`–`009` + `015`)** — the third guard layer next to `BannedSymbols.txt` and the MSBuild structural guards, wired into `Main.csproj` only (tests/benchmarks stay free). Per-call allocation rules (heap/jagged arrays, boxing, capturing closures + instance method groups, interface-typed `foreach`, string building, expanded `params`, `.ToArray()`), plus two convention rules: raw `Parallel.For/ForEach/Invoke` → suppress-aware `OverfitParallel` (a suppress leak measured 1.8× slower inside data-parallel replicas) and direct intrinsics `IsSupported` → the `CpuFeatures` facade. **Severity ladder** in `.editorconfig`: `suggestion` = known debt, `warning` = fix before finishing the feature (hot dirs), **`error` = never merges** (008/015 repo-wide; 001/002/009 in `Kernels/`+`Intrinsics/`). The introduction triage resolved all 45 findings deliberately (13 migrations to `OverfitParallel`, 4 pooled-buffer fixes incl. BatchNorm backward, 4 justified `#pragma`s, rationale-documented downgrades for the CTC family) — **the build sits at 0 warnings**, so a warning means something again. **CI `analyzer-guard` job (guard-of-the-guard)**: drops a deliberate violation into Main and REQUIRES the build to fail with `OVERFIT008` — because a `Microsoft.CodeAnalysis.CSharp` reference newer than the SDK compiler silently disables the whole analyzer (CS9057) while the build stays green (happened for real on a 4.14→5.3 bump vs the SDK's Roslyn 5.0; pinned to 5.0.0 with the verification procedure in `Directory.Packages.props`). `BannedSymbols.txt` +3: `Buffer.BlockCopy` (measured slowest copy variant; both Main sites migrated to `Span.CopyTo`), `string.Intern` (global-table lock contention), `ReaderWriterLock` (obsolete kernel-mode lock). Full catalog + extended proposals: [`Sources/Analyzers/README.md`](Sources/Analyzers/README.md).
...
- **MNIST CNN training: BenchmarkDotNet-grade 503.0 ± 4.2 ms/epoch (DP×8, batch 128) — on par with PyTorch 2.11 CPU at its optimal thread count, consistently ~5–10% faster.** Both sides measured at their optima on the same box/arch/data (`MnistTrainingEpochBenchmark`, 1 op = 1 full 60k epoch, 10 iterations; `Scripts/bench_mnist_torch.py` with a thread sweep — torch's optimum is 8 threads at 524–570 ms/epoch, NOT 16, and 32 threads collapse to 13–14 s/epoch). An earlier "1.2× faster" draft claim compared against torch at 16 threads and was retracted — sweep both sides' knobs before claiming a ratio. **One-cycle LR closes epochs**: warmup→peak→cosine with peak 4–6× the base lr reaches the 5-epoch constant-lr quality in 3 epochs (−47% wall) and BEATS it in 4 (loss 0.0304 vs 0.036–0.049 band, −28% wall) — `MnistOneCycleBenchTests`. Allocation breakdown (`MnistAllocBreakdownTests`): 1.31 MB/epoch = autograd tape objects (754 KB) + trainer/Adam TPL dispatch (656 KB) → GC cost ≤1 ms/epoch (~0.2%), below the benchmark's own stddev — full zero-alloc training would buy nothing measurable and stays an explicit non-goal for performance. Details + retraction note: [`docs/mnist-cnn-training-audit.md`](docs/mnist-cnn-training-audit.md).

## [10.0.24] - 2026-06-11

_Covers everything since 10.0.21 (the 10.0.22 tag shipped without a changelog cut)._
