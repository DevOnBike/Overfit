...
- Honest ceiling (original): pure-managed C# cannot emit every intrinsic llama.cpp uses, but the decode-critical ones (INT8 SIMD) it *can*. The original cautious target was closing 3.75× → ~1.2–1.5×, not exact parity. ⚠️ **An interim Q8 result (13.38 vs 9.67 tok/s, "1.38× faster") flattered Overfit — it compared Overfit-Q8 against LLamaSharp's *FP16* run, not same-file.** The corrected same-file A/B (step 3 status block) puts llama.cpp ~1.5× *ahead* on decode at RAM parity; closing that is the open work. What this step DID nail: the Q8 path tracks F32 in correctness — step 2.5's teacher-forced top-1 test (28/32 greedy match, every flip a genuine near-tie).
