...
| `AN-F1` | **DIAGNOSED** | **the seasonal correction collapses the series' own SCALE, and every relative gate is a percentage of it.** Measured on the lab's actual scatter (7.8% of daily amplitude): typical magnitude goes **10.06 -> 0.156, a 65x collapse**; at 0.2% scatter it is 786x. A gate of "10% of typical" therefore stops meaning 1.0 and starts meaning 0.016 — the guard becomes ~65x more sensitive without anything saying so, which is the shape of a 3x incident rise. **Two competing explanations were eliminated first.** Anchor noise: flat at 134/133/139/139/138/135 across 2-7 days, so more history fixes nothing. Interpolation shape: real and severe — 222 of 284 windows on a perfectly smooth day, against 0 raw — but it decays with scatter and **vanishes above ~1% of amplitude**, and the lab sits at 7.8%, eight times too high. Reproduced deterministically in `SeasonalExpectationNoiseDiagnostics`, no cluster needed. **Remaining is engineering, not research**: the relative gate must be measured against the signal's own scale rather than the residual's — the correction is meant to remove the expected level, not to redefine what "typical" means |
