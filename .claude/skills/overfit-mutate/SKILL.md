...
| **Another agent builds while your mutation is applied** | A mutation to `Sources/Main` is live for the ~60-90 s of the arm, and anything else that compiles the solution in that window links a **deliberately broken** assembly and reports its own results under it. Guard 1 does not see this: it checks *your* target against `HEAD` before you start, not what somebody else does during. Reported as an exposure on 2026-08-16, when five agents were active at once — no incident, but the window was real. **Before mutating shared source, either announce the window, or take the machine-measurement mutex the benchmark host already owns** so concurrent builds are refused with `OVERFITMEASURING` rather than silently poisoned |
| **The victim set SHRINKS when you widen the filter** | Read it as *the added test is wedging the process*, not as *the added test masks the mutation*. Measured 2026-08-16 on `XC-52`: mutation M4 reddened the sequential test **alone** in 24 s; with a concurrency soak added to the same filter the run **hung and named no test at all** (`Test process crashed with exit code -1`, 1.18 GB of dumps), because the soak drove a process-global counter negative, the completion spin stopped terminating and every decode caller in the process blocked — including the test in the other collection. **A mutation arm's FILTER is part of the arm.** When the suite contains a test that can wedge a process-global resource, run the predicted-victim arm with a filter containing **only** the predicted victim; otherwise the verdict is about the process, not about the test |
