...
| 13 | **MCP (Model Context Protocol) — Overfit as HOST** (added 2026-06-11) | 8.5/10 | Bridge `McpTool → ToolDefinition` so `ReActAgent`/`ToolCallConstraint` consume tools from any MCP server (filesystem, DBs, Jira, git…) — the whole MCP ecosystem for free, **stdio = local processes = on-brand zero-egress**. "Private .NET agent with the MCP ecosystem, on-prem" is a real differentiator that completes the agentic story. Official C# SDK exists (`ModelContextProtocol`, co-maintained with Microsoft, integrates with M.E.AI) — verify AOT-compat first; if it fails AOT, hand-roll the minimal stdio/JSON-RPC client (protocol is simple). Ship as a separate package `DevOnBike.Overfit.Mcp` so core stays dependency-free. Secondary (cheaper, later): Overfit as MCP **server** — expose our RAG/LLM as a tool for Claude/other hosts over the existing HttpListener. Scope: ~2-3 sessions; AFTER the bank-demo critical path. |
