...
| `AN-D8` | **PART** | **code done, not deployed.** `OomEventsRate` now reads kube-state-metrics: `increase(restarts_total[w]) * on (namespace,pod,container) group_left() last_terminated_reason{reason="OOMKilled"}` — chosen by measurement against the real kill, not by reasoning: it returned 1.00 for the killed pod and no series for the other eleven, and it **decays** (0 at 2/5/15 min, 1.00 at 30 min), which the reason flag alone does not — that is a state persisting for the container's life and would hold a pod anomalous for ever. **Fixing the template was not enough and that was the trap**: a configured binding overrides it, so every real deployment would have kept the dead series, and deleting the entry is worse (an unbound channel is blind). Added a verbatim `query` field — a join cannot be expressed as name-plus-kind — rejected by the reader unless it carries `%selector%`, or it would silently report on the whole cluster. Also `InertChannels()`: channels that never once moved, split by whether the constant is non-zero (**conclusive** — no real measurement repeats bit-exactly for hours, e.g. latency 48.75) or zero (**ambiguous** — what a healthy rare-event channel looks like). Four mutations, each caught by the right test; suite 2488/2219/269/0. **Remaining: rebuild the guard image and apply the ConfigMap** — that restarts the guard and discards its learned floors, so it is a deliberate act, not a side effect |
